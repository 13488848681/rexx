...
namespace rexx {
    ...
    bool GetPublicKeyElement(const std::string &encode_pub_key, PrivateKeyPrefix &prefix, SignatureType &sign_type,
                             std::string &raw_data) {
    ...
    bool GetKeyElement(const std::string &encode_key, PrivateKeyPrefix &prefix, SignatureType &sign_type,
                       std::string &raw_data) {
        PrivateKeyPrefix prefix_tmp;
        SignatureType sign_type_tmp = SIGNTYPE_NONE;
        std::string buff = DecodeAddress(encode_key);
        if (buff.size() == 29 && (uint8_t) buff.at(0) == 0X9 && (uint8_t) buff.at(1) == 0X5D && (uint8_t) buff.at(2) == 0X13 && (uint8_t) buff.at(3) == 0XD0) {// Address
            prefix_tmp = ADDRESS_PREFIX;
        } else if (buff.size() == 41 && (uint8_t) buff.at(0) == 0Xe3 && (uint8_t) buff.at(1) == 0X2e &&
                   (uint8_t) buff.at(2) == 0Xa9) {//private key
    ...
    std::string PublicKey::GetEncAddress() const {
        ...
        //address Rexx
        str_result.push_back((char) 0X9);
        str_result.push_back((char) 0X5D);
        str_result.push_back((char) 0X13);
        str_result.push_back((char) 0XD0);
    ...
    //not modify
    bool
    PublicKey::Verify(const std::string &data, const std::string &signature, const std::string &encode_public_key) {
        PrivateKeyPrefix prefix;
        SignatureType sign_type;
        ...
        if (sign_type == SIGNTYPE_ED25519) {
            return ed25519_sign_open((unsigned char *) data.c_str(), data.size(), (unsigned char *) raw_pubkey.c_str(),
                                     (unsigned char *) signature.c_str()) == 0;
        } else if (sign_type == SIGNTYPE_CFCASM2) {
            return utils::EccSm2::verify(utils::EccSm2::GetCFCAGroup(), raw_pubkey, "1234567812345678", data,
                                         signature) == 1;
        } else {
    ...
    std::string PrivateKey::Sign(const std::string &input) const {
        ...
        if (type_ == SIGNTYPE_ED25519) {
            ...
            ed25519_sign((unsigned char *) input.c_str(), input.size(), (const unsigned char *) raw_priv_key_.c_str(),
                         (unsigned char *) pub_key_.GetRawPublicKey().c_str(), sig);
            sig_len = 64;
        } else if (type_ == SIGNTYPE_CFCASM2) {
            utils::EccSm2 key(utils::EccSm2::GetCFCAGroup());
    ...
    std::string PrivateKey::GetEncPrivateKey() const {
        ...
        str_result.push_back((char) 0Xe3);
        str_result.push_back((char) 0X2e);
        str_result.push_back((char) 0Xa9);
