...
namespace rexx {
	...
	bool Argument::Parse(int argc, char *argv[]) {
		if (argc > 1) {
            ...
            if (s == "--console-with-cmd") {
				...
				if (argc > 2) {
					printf("%s\n", (utils::Aes::CryptoHex(argv[2], rexx::GetDataSecuretKey())).c_str());
				}
				else {
			...
			//	printf("%s\n", utils::Aes::HexDecrypto(argv[2], rexx::GetDataSecuretKey()).c_str());
			//	return true;
			//}
			else if (s == "--sm3" ) {
				if (argc > 2) {
...
#ifdef SVNVERSION
				printf("%s,%u; " SVNVERSION "\n", General::REXX_VERSION, General::LEDGER_VERSION);
#else
				printf("%s,%u\n", General::REXX_VERSION, General::LEDGER_VERSION);
#endif 
				return true;
	...
	void Argument::ShowNodeId(int argc, char *argv[]) {
		...
		rexx::PrivateKey private_key(utils::Aes::HexDecrypto(argv[2], rexx::GetDataSecuretKey()));
