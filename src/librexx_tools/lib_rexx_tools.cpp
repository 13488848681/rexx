...
REXX_TOOLS_API int InitRexxTools()
{
...
REXX_TOOLS_API void UnInitRexxTools()
{
	utils::Logger::Instance().Exit();
	utils::Logger::ExitInstance();
}

REXX_TOOLS_API int CreateAccountAddress(const char *input_signtype, char *output_result, int *output_len) {
	...
	rexx::SignatureType type = rexx::GetSignTypeByDesc(str_singtype);
	if (type == rexx::SIGNTYPE_NONE) {
		printf("Failed to create account address, signtype \"%s\" error, support: ed25519 or sm2 \n", str_singtype.c_str());
		return -2;
	}

	rexx::PrivateKey priv_key(type);
	if (!priv_key.IsValid()) {
	...
	result["private_key_aes"] = utils::Aes::CryptoHex(private_key, rexx::GetDataSecuretKey());
	result["address"] = public_address;
	result["public_key_raw"] = rexx::EncodePublicKey(priv_key.GetRawPublicKey());
...
REXX_TOOLS_API int CheckAccountAddressValid(const char *input_encode_address) {
	...
	return rexx::PublicKey::IsAddressValid(endcode_address) ? 0 : -2;
}

REXX_TOOLS_API int CreateKeystore(const char *input_password, char *output_keystore, int *output_len) {
...
REXX_TOOLS_API int CheckKeystoreValid(const char *input_keystore, const char *input_password) {
...
REXX_TOOLS_API int SignData(const char *input_privkey, const char *input_rawdata, char *output_data, int *output_len) {
	...
	rexx::PrivateKey priv_key(str_priv_key);
	std::string public_key = priv_key.GetEncPublicKey();
...
REXX_TOOLS_API int SignDataWithKeystore(const char *input_keystore, const char *input_password, const char *input_blob, char *output_data, int *output_len) {
	if (input_keystore == NULL || strlen(input_keystore) == 0) {
	...
	rexx::PrivateKey priv_key(private_key);
	std::string public_key = priv_key.GetEncPublicKey();
...
REXX_TOOLS_API int CheckSignedData(const char *input_blob, const char *input_signeddata, const char *input_pubkey) {
	if (input_blob == NULL || strlen(input_blob) == 0) {
	...
	return rexx::PublicKey::Verify(utils::String::HexStringToBin(str_blob), utils::String::HexStringToBin(str_signeddata), str_pubkey) ? 0 : -2;
}

REXX_TOOLS_API int CreateKeystoreFromPrivkey(const char *input_privkey, const char *input_password, char *output_data, int *output_len) {
	if (input_privkey == NULL || strlen(input_privkey) == 0) {
	...
	rexx::PrivateKey key(str_privkey);
	if (!key.IsValid()) {
...
int GetAddressFromPubkey(const char *input_pubkey, char *output_data, int *output_len) {
	...
	rexx::PublicKey pub_key(str_pubkey);
	...
	result["address"] = public_address;
	result["public_key_raw"] = rexx::EncodePublicKey(pub_key.GetRawPublicKey());
...
REXX_TOOLS_API int GetPrivatekeyFromKeystore(const char *input_keystore, const char *input_password, char *output_data, int *output_len) {
	if (input_keystore == NULL || strlen(input_keystore) == 0) {
