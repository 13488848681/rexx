...
namespace rexx {
	...
	bool LedgerManager::CreateGenesisAccount() {
		...
		AccountFrm::pointer acc_frm =AccountFrm::CreatAccountFrm(Configure::Instance().genesis_configure_.account_, 40000000000000000);
		...
		batch->Put(rexx::General::KEY_LEDGER_SEQ, "1");
		batch->Put(rexx::General::KEY_GENE_ACCOUNT, Configure::Instance().genesis_configure_.account_);
	...
	void LedgerManager::CreateHardforkLedger() {
		...
		do {
			...
			rexx::KeyValueDb *ledger_db = rexx::Storage::Instance().ledger_db();
			...
			batch->Put(rexx::General::KEY_LEDGER_SEQ, utils::String::ToString(header->seq()));
			batch->Put(rexx::General::LAST_PROOF, "");

			ValidatorsSet(batch, new_validator_set);

			//Write ledger db
			WRITE_BATCH batch_ledger;
			batch_ledger.Put(rexx::General::KEY_LEDGER_SEQ, utils::String::ToString(header->seq()));
	...
	bool LedgerManager::CloseLedger(const protocol::ConsensusValue& consensus_value, const std::string& proof) {
		...
		account_db_batch->Put(rexx::General::KEY_LEDGER_SEQ, utils::String::Format(FMT_I64, ledger_seq));
		...
		//proof
		account_db_batch->Put(rexx::General::LAST_PROOF, proof);
		account_db_batch->Put(rexx::General::STATISTICS, statistics_.toFastString());
		proof_ = proof;
	...
	void LedgerManager::OnRequestLedgers(const protocol::GetLedgers &message, int64_t peer_id) {
		...
		if (ret) {
			rexx::WsMessagePointer ws = std::make_shared<protocol::WsMessage>();
	...
	Result LedgerManager::DoTransaction(protocol::TransactionEnv& env, LedgerContext *ledger_context) {
		...
		TransactionFrm::pointer txfrm = std::make_shared<rexx::TransactionFrm >(env);
