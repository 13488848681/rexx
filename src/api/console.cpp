...
namespace rexx {
	...
	void Console::GetBalance(const utils::StringVector &args) {
		...
		if (!Environment::AccountFromDB(address, account_ptr)) {
			...
			std::cout << utils::String::FormatDecimal(account_ptr->GetAccountBalance(), General::REXX_DECIMALS ) << " Rexx"<<std::endl;
		}
	}
	...
	void Console::GetState(const utils::StringVector &args) {
		...
		do {
			...
			reply_json = *rexx::StatusModule::modules_status_;
		} while (false);
	...
	void Console::PayCoin(const utils::StringVector &args) {
		...
		if (priv_key_ != NULL) {
			...
			if (!utils::String::IsDecNumber(args[2], General::REXX_DECIMALS)) {
				std::cout << "error, amount not valid" << std::endl;
				return;
			}

			if (!utils::String::IsDecNumber(args[3], General::REXX_DECIMALS)) {
				std::cout << "error, fee not valid" << std::endl;
				return;
			}

			int64_t coin_amount = utils::String::Stoi64(utils::String::MultiplyDecimal(args[2], General::REXX_DECIMALS));
			int64_t fee_limit = utils::String::Stoi64(utils::String::MultiplyDecimal(args[3], General::REXX_DECIMALS));
