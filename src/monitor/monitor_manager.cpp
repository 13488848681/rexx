...
namespace rexx {
	MonitorManager::MonitorManager() : Network(SslParameter()) {
		...
		request_methods_[monitor::MONITOR_MSGTYPE_REXX] = std::bind(&MonitorManager::OnRexxStatus, this, std::placeholders::_1, std::placeholders::_2);
	...
	rexx::Connection * MonitorManager::CreateConnectObject(rexx::server *server_h, rexx::client *client_, rexx::tls_server *tls_server_h,
		rexx::tls_client *tls_client_h, rexx::connection_hdl con, const std::string &uri, int64_t id) {
	...
	bool MonitorManager::OnMonitorHello(protocol::WsMessage &message, int64_t conn_id) {
		...
		do {
			...
			reg.set_blockchain_version(rexx::General::REXX_VERSION);
			reg.set_data_version(rexx::General::MONITOR_VERSION);
	...
	bool MonitorManager::OnRexxStatus(protocol::WsMessage &message, int64_t conn_id) {
		monitor::RexxStatus rexx_status;
		// Get the status of rexx
		GetRexxStatus(rexx_status);
		...
		// Send the response of rexx status
		if (NULL == monitor || !monitor->SendResponse(message, rexx_status.SerializeAsString(), ignore_ec)) {
			bret = false;
			LOG_ERROR("Failed to send rexx status from ip(%s) (%d:%s)", monitor->GetPeerAddress().ToIpPort().c_str(),
				ignore_ec.value(), ignore_ec.message().c_str());
		}
		return bret;
	}
	...
	bool MonitorManager::GetRexxStatus(monitor::RexxStatus &rexx_status) {
		...
		monitor::GlueManager *glue_manager = rexx_status.mutable_glue_manager();
		...
		monitor::PeerManager *peer_manager = rexx_status.mutable_peer_manager();
