...
namespace rexx {
	...
	class MonitorManager :public utils::Singleton<MonitorManager>,
		...
		friend class utils::Singleton<rexx::MonitorManager>;
	public:
		MonitorManager();
		~MonitorManager();
	...
	private:
		...
		*************************************************/
		...
		Input:          server_h rexx::server* The http server
						client_ rexx::client* The http client
						tls_server_h rexx::tls_server* The tls server
						tls_client_h rexx::tls_client* The tis client
						con rexx::connection_hdl The handle of connection
						uri const std::string The url
						id int64_t The id of connection
		Return:         The connection of websocket
		*************************************************/
		virtual rexx::Connection *CreateConnectObject(rexx::server *server_h, rexx::client *client_,
			rexx::tls_server *tls_server_h, rexx::tls_client *tls_client_h,
		...
		*************************************************/
		...
		/*************************************************
		Function:       OnRexxStatus
		Description:    The monitor to rexx status message
		Calls:          SendRequest
		Input:          message protocol::WsMessage The message of monitor
		                conn_id int64_t The id of connection
		Return:         bool success or failure
		*************************************************/
		bool OnRexxStatus(protocol::WsMessage &message, int64_t conn_id);
		...
		*************************************************/
		...
		/*************************************************
		Function:       GetRexxStatus
		Description:    Get the status of rexx
		Calls:          GetPeers
		Input:          rexx_status monitor::RexxStatus The status of rexx
		Return:         bool success or failure
		*************************************************/
		bool GetRexxStatus(monitor::RexxStatus &rexx_status);
